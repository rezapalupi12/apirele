...
class Controller {
    ...
    static create = async (req, res, next) => {
        ...
        //const alat = 'Rele'
        try {
            const { ip, status, time, timedate, durasidown } = matchedData(req);
            let results = [];
            if (status == 'down') {
                ...
                const [insert] = await DB.execute(
                    "INSERT INTO `his_rele` (`ip`, `tanggal_down`, `waktu_down`) VALUES (?,?,?)",
                    [ip,tanggal,time] 
                );
                const [update] = await DB.execute(
                    "UPDATE `real_rele` SET `status` = ? where `ip` = ?",
                    [status,ip]
                );
                const [update1] = await DB.execute(
                    "CALL updatesite_rele()"
                );
                res.status(201).json({
                    ok: 1,
                    status: 201,
                ...
                if(down_time_key && tanggal_down){
                    ...
                    if(monthDifference === 0){
                        ...
                        const [update] = await DB.execute(
                            "UPDATE `his_rele` SET `tanggal_up` = ?, `waktu_up`=?, `durasi_down`=? where `ip`=? and `tanggal_down`=? and `waktu_down`=?",
                            [tanggal, time, durasidown,  ip, tanggal_down, down_time_key]
                        );
                        const [update1] = await DB.execute(
                            "UPDATE `real_rele` set `status`=? where `ip`=?",
                            [status,ip]
                    ...
                    const [tableupdate1] = await DB.execute(
                        "UPDATE `real_rele` SET `link_status` = ? WHERE `ip` = ?",
                        [status, ip]
                    );
                    results.push({
                        operation: "update1",
                    ...
                    for (var i = 1; i <= monthDifference; i++) {
                        ...
                        if (i === 1) {
                            const [result1] = await DB.execute(
                                "UPDATE `his_rele` SET  `Tanggal_Up`=?, `Waktu_Up`=?  WHERE `Tanggal_Down`=? AND `Waktu_Down`=?  and `ip` = ?",
                        ...
                        const [insertupdate] = await DB.execute(
                            "INSERT INTO `his_rele` (`alat`, `ip`, `Tanggal_Down`, `Waktu_Down`, `Tanggal_Up`, `Waktu_Up`) VALUES (?,?,?,?,?,?)",
